...
- **实现内容**（当前阶段符合“先简化：JSON 导入/导出 + 生成卡牌”的预期）：
  - 新增调试面板 Tab：**模组/卡牌**
  - 支持粘贴 `UnitTemplate` JSON 并**添加/更新**到当前 `BattleConfig.unitTemplates`
  - 支持选择模板并生成 `BattleCard` 副本（按数量），并可注入到我方/敌方的**手牌或牌堆**
  - 支持导出当前 `unitTemplates + battleCards` 的 JSON（用于复制/保存，后续可演进为 mod.json）
- **下一步建议**：
  - 将生成入口从“战斗内 BattleCard 注入”扩展到“战役外 CardInventory（背包）→ createBattleCardsFromInventory”
  - 建立 `mod.json` 的最小 schema 与加载/合并流程（解决 ID 冲突、版本号、启用/禁用）
  - 编辑器增强：模板表单化编辑、校验（必填字段、ID 唯一性、数值范围）
